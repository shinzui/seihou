...
**removable** (Bool): Whether this module can be removed with `seihou remove`. Defaults to `False`. Set to `True` only when all of the module's effects are reversible — meaning its steps produce standalone files (Copy, Template, or DhallText strategies) without patch operations, and it has no destructive shell commands. When `True`, users can run `seihou remove <module>` to delete all files generated by this module and remove it from the manifest. See [Removing modules](#removing-modules).
...
Modules can opt in to being removable by setting `removable = True` in their `module.dhall`. This allows users to undo the module's effects with `seihou remove <module>`.
...
### When to set removable = True

A module is safe to mark as removable when:

- All steps use **Copy**, **Template**, or **DhallText** strategies (standalone files)
- No steps use **patch** operations (append-file, prepend-file, append-section)
- No steps use the **Structured** strategy (records from multiple modules are merged and cannot be separated)
- Shell **commands** are non-destructive or idempotent

### When to keep removable = False (the default)

Leave `removable = False` when:

- The module contributes content to files created by other modules (via patch operations)
- The module uses the Structured strategy where records are deep-merged with other modules
...
1. The manifest is checked to confirm the module was applied and is removable.
2. Each file generated by the module is classified: unchanged (safe to delete), modified by user (conflict), or already deleted.
3. The removal plan is shown. Safe files are deleted; conflicted files prompt for keep/delete.
4. The manifest is updated to remove the module and its file records.
5. Empty parent directories are cleaned up automatically.

Use `--dry-run` to preview the removal plan without making changes, and `--force` to skip conflict prompts.
