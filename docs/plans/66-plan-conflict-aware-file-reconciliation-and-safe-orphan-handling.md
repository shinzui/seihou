...
- [x] (2026-07-19 18:41Z) M3: Run all validation and record a temporary-project reconciliation transcript.
...
EP-66 delivered one update-specific file model from rendering through recovery. Ordered
writes, copies, and patches now materialize into one `DesiredFile` per path; baseline,
disk, and generated bytes classify safe updates, preserved user edits, Git merges,
conflicts, jointly owned paths, and every orphan state. Resolutions retain both generated
and applied content, so KeepCurrent advances the ancestor without erasing customization.

`UpdateTransaction` snapshots every managed path before mutation, rejects stale or
unresolved plans, writes through same-directory temporary files, uses EP-65's baseline
store, returns an updated candidate manifest, and restores interrupted work. Its journal
stores the candidate manifest so startup recovery can distinguish a published update from
one that must roll back. Malformed journals move to quarantine instead of being destroyed.

The disposable real-filesystem fixture exercised the production Git driver, baseline
store, planner, resolver, and transaction together. Its observed result was:

```text
Files: 1 merged; 1 deleted; 1 edited orphan retained; 0 conflicts
Merged file: user edit and generated edit both present
Safe orphan: absent from disk and candidate manifest
Edited orphan: present on disk and still tracked
Transaction journals remaining: 0
```

Validation passed with 996 core tests, 262 CLI tests, and 16 extension tests. `nix fmt`
and `git diff --check` also passed. EP-68 still owns module fetching, migration/command
coordination, durable manifest publication, and transaction completion; arbitrary command
side effects remain outside this managed rollback boundary as planned.
