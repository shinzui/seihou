...
calls Baikai providers, `Seihou.CLI.AgentConfig` resolves provider and
model configuration, and `Seihou.CLI.AgentLaunch` gathers context and
formats shared prompt sections. The executable handlers import those
library modules after embedding their command-specific prompt
templates.
