...
data DiffResult = DiffResult
  ...
  , diffOrphaned   :: [OrphanedFile]    -- In manifest (active module), not in plan
...
`computeDiff` accepts a `Set ModuleName` of **active modules** (the modules in the current
composition). Only manifest files owned by an active module participate in the diff. Files
owned by inactive modules are invisible — preserved in the manifest but not classified.

For each file path across all three sources (scoped to active modules):

| Manifest (active) | Plan | Disk | Classification |
|---|---|---|---|
...
| present | absent | present | **Orphaned** — active module no longer produces this file |
...
| Active module no longer produces a file | Its files become Orphaned in status |
| Module not in current composition | Its files are preserved in manifest, invisible to diff |
...
| Orphaned file detection | Unit | In active manifest, not in plan → Orphaned |
| Inactive module preservation | Unit | Files from inactive modules not classified as orphaned |
