...
Set `removable = True` when a module's effects are safely reversible (Copy, Template, or DhallText strategies only, no patch operations, no destructive commands). Users can then run `seihou remove <module>` to delete generated files and remove the module from the manifest.
