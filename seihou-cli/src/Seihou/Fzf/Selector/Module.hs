...
-- | Discover all available modules and present them in an fzf picker.
selectModule :: (Fzf :> es, IOE :> es) => Eff es (FzfResult ModuleName)
selectModule = do
  searchPaths <- liftIO defaultSearchPaths
  discovered <- liftIO (discoverAllModules searchPaths)
  let candidates = mapMaybe formatModuleCandidate discovered
